...
/// Move worksheet to before ohter.
/// </summary>
public class SetWorkSheetMoveBefore : SetBase
{
    #region constructor/s

    /// <summary>
    /// Initializes a new instance of the <see cref="SetWorkSheetMoveBefore"/> class.
    /// </summary>
    public SetWorkSheetMoveBefore()
    {
    ...
    /// <summary>
    /// Gets or sets a reference to move the referenced sheet just before.
    /// </summary>
    /// <value>
    /// A <see cref="T:System.String"/> containing the name of the sheet that acts as a reference to move the referenced sheet just before.
    /// </value>
    public string BeforeSheetName { get; set; }
    ...
    public override ActionResult Execute(IXlsxInput context, Stream input, ExcelPackage package, ExcelWorksheet worksheet)
    {
        ...
        {
            return ActionResult.CreateErrorResult(
                "Before sheet name can not be null or empty",
                new ActionResultData
                {
                    Context = context,
                    InputStream = input,
        ...
        return ExecuteImpl(context, input, package, worksheet, BeforeSheetName);
    }
    ...
    private static ActionResult ExecuteImpl(IXlsxInput context, Stream input, ExcelPackage package, ExcelWorksheet worksheet, string beforeSheetName)
    {
        var outputStream = new MemoryStream();

        try
        {
            worksheet.Workbook.Worksheets.MoveBefore(worksheet.Name, beforeSheetName);
