...
/// <summary>
/// Shows how to copy ranges asynchronously.
/// </summary>
internal class Sample20
{
    // Generates document
    public static async Task GenerateAsync(ILogger logger, CancellationToken cancellationToken = default)
    {
        ...
        XlsxInput doc = XlsxInput.Create(new[] { "Sheet1", "Sheet2" });
        ...
        logger.Info("     > Path: ~/Output/Sample-20/Sample-20.xlsx");
        logger.Info($"   > Elapsed time: { ts.Hours:00}:{ ts.Minutes:00}:{ ts.Seconds:00}.{ ts.Milliseconds / 10:00}");
