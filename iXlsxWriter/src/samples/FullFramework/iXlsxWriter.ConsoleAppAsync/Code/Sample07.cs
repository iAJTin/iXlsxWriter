...
/// <summary>
/// Shows how to save a xlsx input as zip file asynchronously.
/// </summary>
internal class Sample07
{
    public static async Task GenerateAsync(ILogger logger, CancellationToken cancellationToken = default)
    {
        ...
        var result = await doc.CreateResultAsync(XlsxOutputResultConfig.ZippedResult("custom_zipfile_name"), cancellationToken).ConfigureAwait(false);
        if (!result.Success)
        {
        ...
        logger.Info("     > Path: ~/Output/Sample-07/Sample-07.xlsx");
        logger.Info($"   > Elapsed time: { ts.Hours:00}:{ ts.Minutes:00}:{ ts.Seconds:00}.{ ts.Milliseconds / 10:00}");
